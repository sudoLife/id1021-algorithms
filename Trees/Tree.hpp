...
public:
    TreeIterator(PointerType root) : current(root), last(root)
    {
        // end()
        if (current == nullptr)
        {
    ...
    Iterator &operator++()
    {
        if (current == last) // end of the road
            current = nullptr;
        if (current == nullptr)
        {
            return *this;
        }
        // descend leftmost of right branch, skip pushing "visited" node
        if (current->right)
        {
...
private:
    PointerType current;
    PointerType last;
    DynamicStack<PointerType> stack;
};
